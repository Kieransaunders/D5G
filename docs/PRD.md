...
| **Plaintext key stored** in `d5g_api_key_plain` alongside the hash (`Auth.php:22`) | **Open.** Powers the "show my key" admin UI; hashing alone is then a half-truth. Decide: drop the plaintext copy (key shown once at generation) or stop describing auth as hashed. |
...
1. **Freemius init is copy-pasted from Airloop** — `divi5-generator.php` has `'premium_slug' => 'Airloop-premium'` and product id `33991`. If that id is Airloop's Freemius product, licensing will activate against the wrong product. Must be its own Freemius product with slug `divi5-generator-premium`.
...
4. ~~**No licence enforcement in code:** every REST endpoint is available to anyone with the API key.~~ **Closed in code, blocked on Freemius** — `pro_gate()` + `requires_pro()` in `RestApi.php` return `403 pro_required`, landed via PR #28 and covered by `RestApiProGateTest.php`. It gates nothing in reality until the real Freemius product exists (gap 1) — `is_pro()` currently resolves against Airloop's product id.

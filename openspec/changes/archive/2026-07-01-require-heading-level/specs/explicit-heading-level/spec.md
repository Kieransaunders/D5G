...
A new `heroHeading(opts)` helper MUST emit a heading block with `headingLevel: 'h1'`. It
MUST ignore any `level` option passed, so the single hero h1 cannot be mis-marked.

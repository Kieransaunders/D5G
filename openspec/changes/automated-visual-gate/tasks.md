...
Turns green: T1-T4
- [x] 1.1 `diff(a, b)` — dependency-free; throw on width/height mismatch; count pixels
      whose max channel difference exceeds a per-pixel threshold; return
      `{mismatchPixels, total, mismatchPct}` [test: T1, T2, T3]
- [x] 1.2 `gate(mismatchPct, maxMismatch=0.05)` — return `{pass, mismatchPct, maxMismatch}`
      with pass = mismatchPct <= maxMismatch [test: T1, T2, T4]

## 2. CLI
- [x] 2.1 `node visual-diff.js <a.png> <b.png> [--max-mismatch 0.05]` — load two PNGs
...
- [x] 3.1 Rewrite Stage 4 to run `visual-diff.js` (live Playwright screenshot vs Stage 2
      mockup screenshot), blocking delivery on FAIL — replacing manual "compare against the
      mockup" [test: spec scenario]

## 4. Verification
- [x] 4.1 `visual-diff.test.js` all green (4 assertions)
- [x] 4.2 Full skill suite stays green; SKILL.md ≤ 150 lines

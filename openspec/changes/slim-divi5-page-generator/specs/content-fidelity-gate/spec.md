...
Prior art: parked `tasks.md` (L1/L2/L3 mockup-fidelity gate). L1 + L2 ship here.
L3 (Playwright render) is deferred to a follow-up change.

## ADDED Requirements

### Requirement: fidelity-check.js performs L1 content fidelity checks
A new script `scripts/fidelity-check.js` MUST compare the Stage-2 HTML mockup against
the generated page JSON at the data level (no browser render). It MUST exit 1 on FAIL.

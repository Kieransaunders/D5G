...
The `GET /wp-json/divi-tools/v1/ping` response SHALL include a `seo_plugin` field whose value is the detected plugin id (`yoast`, `rank_math`, `aioseo`, `seopress`, `tsf`) or `null` when no supported plugin is active.

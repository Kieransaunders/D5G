...
Turns green: T1, T3
- [ ] 1.1 Insert a "Stage 0.5 — Resolve preset library" step between Stage 0 and Stage 1:
...
- [ ] 1.2 Point at `examples/preset-first-workflow.js` as the reference sequence
      (registry → presetRef → import) [test: T3]

## 2. Rewrite the reused-style rule (rule 4)
Turns green: T2
- [ ] 2.1 Change rule 4 from "every reused style is a `builder.preset()`" to: build every
...
- [ ] 3.1 Confirm line count ≤ 150 after the additions (the slim-skill constraint) [test: T4]

## 4. Verification
- [ ] 4.1 `preset-first.test.js` all green (4 assertions)
- [ ] 4.2 Full skill suite stays green (no regression to slim-skill / clone-first /

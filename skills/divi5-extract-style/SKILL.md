...
description: "Divi 5 style extraction and variables tool: (1) brand guide / design tokens into importable Divi 5 Global Variables JSON; (2) an existing Divi 5 page export FILE into design tokens (colours, variables, presets) the page generator can reuse. Use when converting a style guide into Divi 5 Global Variables, or extracting tokens from a Divi 5 export JSON file (for a live site instead, use brand-extract). Triggers: extract divi style, reuse divi design, divi export to tokens, divi global variables, divi global colors, style guide to divi, design tokens divi, reuse divi design system, extract divi presets, divi export design system."

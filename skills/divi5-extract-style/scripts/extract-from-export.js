...
/**
 * Parse a Divi 5 export doc into structured colours/variables/presets.
 * Pure function: takes the parsed JSON, returns plain objects. Shared by the
 * CLI below and by extractBrandProfileFromExport().
 */
function parseExport(doc) {
  ...
  // Resolve a colour id to a concrete hex where possible (follows derivation chain).
  ...
  return { rawColors, rawVars, colors, colorById, resolveHex, variables };
}

/**
 * Collect presets (module + group) as flat {id,name,kind,...,attrs} records.
 * `attrs` retained so font extraction can read preset typography settings.
 */
...
/**
 * Extract a Brand Profile JSON from a Divi 5 export doc.
 * Returns the canonical shape consumed by the app's Brand editor + the
 * divi5-brand-profile skill:
 *   { name, colors:[{role,hex,source,locked,id}], fonts:{heading?,body?},
 *     logo, voice, tagline, sourceType, sourceRef, extractedAt }
 *
 * Colours come from global_colors (resolved through any derivation chain); each
 * keeps its gcid- id so a generated page can re-use the existing global colour.
 * Fonts are best-effort: pulled from preset typography attrs when present.
 */
function extractBrandProfileFromExport(doc, opts = {}) {
  const { colors, resolveHex } = parseExport(doc);

  const brandColors = colors
    .filter(c => c.status !== 'archived')
    .map((c, i) => ({
      role: i === 0 ? 'primary' : i === 1 ? 'accent' : `color-${i + 1}`,
      hex: resolveHex(c.id) || c.raw || '#000000',
      source: 'export',
      locked: false,
      id: c.id,
      label: c.label,
  ...
  // Fonts: scan preset attrs for a font family. Prefer a heading/text module,
  // then fall back to the first two distinct families found anywhere in presets.
  const presetList = [
    ...collectPresets(doc.presets && doc.presets.module, 'module'),
    ...collectPresets(doc.presets && doc.presets.group, 'group'),
  ];
  function familyFromAttrs(attrs) {
    if (!attrs || typeof attrs !== 'object') return null;
    const f = attrs.font || attrs.typography;
    if (f && typeof f === 'object' && f.family) return f.family;
    return null;
  }
  const fonts = {};
  const headingPreset = presetList.find(p => /head|title|h1/i.test(p.moduleName + p.name));
  const bodyPreset = presetList.find(p => /text|body|paragraph/i.test(p.moduleName + p.name));
  if (headingPreset) {
    const fam = familyFromAttrs(headingPreset.attrs);
    if (fam) fonts.heading = { family: fam, source: 'export' };
  }
  if (bodyPreset) {
    const fam = familyFromAttrs(bodyPreset.attrs);
    if (fam) fonts.body = { family: fam, source: 'export' };
  }
  // Generic fallback: first two distinct families found in any preset.
  if (!fonts.heading || !fonts.body) {
    const seen = [...new Set(presetList.map(p => familyFromAttrs(p.attrs)).filter(Boolean))];
    if (!fonts.heading && seen[0]) fonts.heading = { family: seen[0], source: 'export' };
    if (!fonts.body && (seen[1] || seen[0])) fonts.body = { family: seen[1] || seen[0], source: 'export' };
  }
...
// Export the pure helpers for tests + reuse. The CLI block below only runs when
// this file is executed directly (not require()'d).
module.exports = { parseExport, collectPresets, extractBrandProfileFromExport, parseVariableRef };
...
if (require.main === module) {
  ...
  const { rawColors, rawVars, colors, colorById, resolveHex, variables } = parseExport(doc);

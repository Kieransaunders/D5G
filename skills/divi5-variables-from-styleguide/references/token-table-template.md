...
If you can supply tokens in this table format, the skill can generate Divi variables
deterministically. Paste it into the prompt or point `--tokens` at a CSV/JSON of the same
shape.
...
| `layout.container.max` | `numbers` | `1200px` | Max content width |
| `link.site` | `links` | `https://example.com` | Canonical URL |

## Notes

- Colours in this table become BOTH a `global_colors` entry AND a `colors`-type
  `global_variables` entry (so every colour is referenceable by presets).
- Leave a row out rather than guessing a value — the skill never invents values.

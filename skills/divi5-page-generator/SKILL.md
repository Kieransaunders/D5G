...
**Headless/brief mode:** if a complete brief is supplied in the prompt, via slash-command arguments ($ARGUMENTS), or a `brief.json` exists in the working directory, run **fully autonomously to file delivery** — there is no human to respond, so you MUST NOT stop to ask anything. Specifically: skip all Stage-1 questions, AND **skip the Stage-2 approval gate** — build the HTML preview, run the taste pre-flight against it yourself, fix the misses, then proceed straight into Stage 3 and write the output files. Never end your turn with a question like *"Does this look right? Approve and I'll generate the JSON"* — in headless mode that silently produces no page. The run is **only complete when a validated `[brand]-landing-page.json` (valid `et_builder` JSON, not HTML) exists on disk**; if you stop before that, the generation has failed.
...
Build a complete styled HTML page (`preview-[brand].html`) applying the design system in `references/aesthetics.md`, the layout recipes in [references/layout-patterns.md](references/layout-patterns.md), the SEO copy rules in `references/seo.md`, and the design judgement in [references/taste.md](references/taste.md). The HTML *is* the design spec — the JSON must match it. **In interactive mode:** serve it, screenshot it, ask for approval, iterate until approved. **In headless/brief mode:** do NOT ask for approval — run the taste pre-flight yourself, fix the misses, and continue straight to Stage 3 in the same turn.
...
| Placeholder images don't load on live sites | `loremflickr.com` (intermittently returns nothing → blank image columns on a real domain) | Use a host that reliably returns an image — `https://picsum.photos/{w}/{h}?random={n}` loaded fine on the live test site. `validate.js` WARNs on any `loremflickr`/`picsum`/`placehold` src so placeholders can't ship unflagged. Always swap for real photography before launch |

...
class LegacyPayload_Test extends TestCase {
	...
	public function test_legacy_payload_on_yoast_matches_pre_1_6_writes(): void {
		...
		$result = D5G_SeoWriter::write( self::POST_ID, $legacy );
	...
	public function test_legacy_payload_on_rankmath_matches_pre_1_6_writes(): void {
		D5G_Seo_Detector::set_signals( array( 'rank_math' => true ) );
	...
	public function test_legacy_payload_with_only_title_and_description_runs_warning_free(): void {
		...
		D5G_SeoWriter::write( self::POST_ID, array(
			'title'       => 'T',
			'description' => 'D',
		) );
	...
	public function test_no_plugin_active_falls_back_to_d5g_keys(): void {
		D5G_Seo_Detector::set_signals( array() );

		$result = D5G_SeoWriter::write( self::POST_ID, array(
			'title'       => 'T',
			'description' => 'D',
			'keyword'     => 'kw',
		) );
		...
		$this->assertSame( 'T', MetaStore::get( self::POST_ID, '_d5g_seo_title' ) );
		$this->assertSame( 'D', MetaStore::get( self::POST_ID, '_d5g_seo_description' ) );
		$this->assertSame( 'kw', MetaStore::get( self::POST_ID, '_d5g_seo_focuskw' ) );
	}

...
/**
 * @covers \D5G_Seo_Yoast
 * @covers \D5G_Seo_RankMath
 * @covers \D5G_Seo_AIOSEO
 * @covers \D5G_Seo_SEOPress
 * @covers \D5G_Seo_TSF
 * @covers \D5G_Seo_Fallback
 *
...
class AdapterKeyMap_Test extends TestCase {
	...
	public function test_yoast_full_payload_writes_native_keys(): void {
		$result = ( new D5G_Seo_Yoast() )->write( self::POST_ID, $this->full_payload() );
	...
	public function test_yoast_single_keyword_does_not_write_focuskeys_array(): void {
		...
		( new D5G_Seo_Yoast() )->write( self::POST_ID, $payload );
	...
	public function test_rankmath_full_payload_writes_native_keys(): void {
		$result = ( new D5G_Seo_RankMath() )->write( self::POST_ID, $this->full_payload() );
	...
	public function test_aioseo_writes_flat_keys_and_envelope(): void {
		$result = ( new D5G_Seo_AIOSEO() )->write( self::POST_ID, $this->full_payload() );
	...
	public function test_aioseo_envelope_merge_preserves_existing_keyphrase(): void {
		...
		( new D5G_Seo_AIOSEO() )->write( self::POST_ID, array( 'title' => 'New Title' ) );
	...
	public function test_seopress_full_payload_writes_native_keys(): void {
		$result = ( new D5G_Seo_SEOPress() )->write( self::POST_ID, $this->full_payload() );
	...
	public function test_tsf_full_payload_writes_native_keys(): void {
		$result = ( new D5G_Seo_TSF() )->write( self::POST_ID, $this->full_payload() );
		...
		$this->assertSame( 'invoice software', MetaStore::get( self::POST_ID, '_d5g_seo_focuskw' ) );
	...
	public function test_fallback_writes_neutral_keys(): void {
		$result = ( new D5G_Seo_Fallback() )->write( self::POST_ID, $this->full_payload() );

		$this->assertNull( $result['plugin'], 'Fallback reports null plugin' );
		$this->assertSame( 'Invoice Software for Small Business', MetaStore::get( self::POST_ID, '_d5g_seo_title' ) );
		$this->assertSame( 'invoice software', MetaStore::get( self::POST_ID, '_d5g_seo_focuskw' ) );
		// Fallback does not persist OG/Twitter/canonical/robots.
		$this->assertFalse( MetaStore::has( self::POST_ID, '_d5g_seo_og_title' ) );
	}

	public function test_empty_payload_writes_nothing_in_any_adapter(): void {
		foreach ( array(
			new D5G_Seo_Yoast(),
			new D5G_Seo_RankMath(),
			new D5G_Seo_AIOSEO(),
			new D5G_Seo_SEOPress(),
			new D5G_Seo_TSF(),
			new D5G_Seo_Fallback(),
		) as $adapter ) {
			MetaStore::reset();

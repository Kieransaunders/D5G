 ...
 * focus keyword is persisted to the neutral _d5g_seo_focuskw key for
 * traceability (it surfaces in GET /pages design_hint if no title is present).
 */
class D5G_Seo_TSF extends D5G_Seo_AdapterBase {
	...
	public function detect(): bool {
		return D5G_Seo_Detector::signal( 'tsf' );
	}
		...
		if ( ! empty( $seo['robots'] ) && is_array( $seo['robots'] ) ) {
			if ( isset( $seo['robots']['noindex'] ) ) {
				if ( $seo['robots']['noindex'] ) {
					...
					$this->clear( $page_id, '_genesis_noindex', 'robots.noindex', $written );
				}
			}
			if ( isset( $seo['robots']['nofollow'] ) ) {
				if ( $seo['robots']['nofollow'] ) {
					...
					$this->clear( $page_id, '_genesis_nofollow', 'robots.nofollow', $written );
				}
			}
		}

...
/**
 * Imports a SQL dump produced by D5G_DbExporter, then rewrites the source
...
class D5G_DbImporter {
	...
	public static function import( string $sql, string $from_url, string $to_url ): array {
		...
		$keep_home = home_url();
		$keep_site = site_url();
		$keep_key  = get_option( D5G_Auth::KEY_OPTION );
		...
		if ( $keep_key ) {
			self::restore_option( D5G_Auth::KEY_OPTION, $keep_key, 'no' );
		}
		wp_cache_flush();
	 ...
	 *  ponytail: not a general SQL parser; pairs only with D5G_DbExporter output. */

...
class D5G_SettingsPage {
  ...
  -d '{
	 ...
	 * the public D5G marketplace — the sole canonical install path (the
	 * Freemius Add-Ons zip alternative was retired: it shipped full toolkit
	 * source as a raw downloadable zip). Returned as a string (not echoed) so
	 * both branches are unit-testable without toggling D5G_Limits::is_pro().
	 */
	public static function install_instructions_html( bool $is_pro ): string {
		...
		if ( $is_pro ) :
			...
			<pre style="background:#1e1e1e;color:#d4d4d4;padding:16px;border-radius:6px;overflow-x:auto;font-size:12px">claude plugin marketplace add <?php echo esc_html( self::PRO_MARKETPLACE ); ?></pre>

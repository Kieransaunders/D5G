 ...
 *
 * The order is filterable via `dti/seo/adapter_order` (return a reordered
...
class DTI_Seo_Detector {
	...
	private static function adapters(): array {
		return array(
			'rank_math' => DTI_Seo_RankMath::class,
			'yoast'     => DTI_Seo_Yoast::class,
			'aioseo'    => DTI_Seo_AIOSEO::class,
			'seopress'  => DTI_Seo_SEOPress::class,
			'tsf'       => DTI_Seo_TSF::class,
			'fallback'  => DTI_Seo_Fallback::class,
		);
	}
	 ...
	 */
	public static function resolve(): DTI_Seo_Adapter {
	...
	/**
	 * Apply the dti/seo/adapter_order filter. Ensures 'fallback' is always
	 * last and that any unknown ids from the filter are dropped.
	 */
	private static function ordered_ids( array $default_ids ): array {
		$filtered = apply_filters( 'dti/seo/adapter_order', $default_ids );
